...
async function ensureSession() {
    ...
    starting.value = true;
    try {
        const res = await api.post<{ id: string }>('/api/v1/customer/support/chat/session', {});
...
async function fetchMessages(initial = false) {
    ...
    try {
        ...
        const res = await api.get<{ messages: ChatMessage[] }>(`/api/v1/customer/support/chat/${sessionId.value}/messages${q}`);
        const incoming = res.messages ?? [];
        if (initial) messages.value = incoming;
        else if (incoming.length) {
            messages.value.push(...incoming);
            if (!open.value) unread.value += incoming.filter((m) => m.sender_actor_type !== 'customer').length;
        }
...
async function send(text?: string) {
    ...
    await ensureSession();
    // optimistic
    messages.value.push({ id: `tmp-${Date.now()}`, sender_actor_type: 'customer', body, kind: 'text', created_at: new Date().toISOString() });
    draft.value = '';
    scrollToBottom();
    try {
        await api.post(`/api/v1/customer/support/chat/${sessionId.value}/messages`, { body });
        await fetchMessages();
    } catch { /* ignore */ } finally {
        sending.value = false;
    }
...
async function escalate() {
    ...
    try {
        const res = await api.post<{ reference: string }>(`/api/v1/customer/support/chat/${sessionId.value}/escalate`, { subject: subject.trim() });
...
function who(m: ChatMessage) {
    if (m.sender_actor_type === 'customer') return 'You';
      ...
      <section v-if="open" class="cw-panel" role="dialog" aria-label="Support chat">
        ...
        <div ref="scroller" class="cw-body">
          ...
          <div v-for="m in messages" :key="m.id"
               :class="['cw-msg', m.sender_actor_type === 'customer' ? 'cw-msg--mine' : m.kind === 'system' ? 'cw-msg--sys' : 'cw-msg--them']">
...
<style scoped>
.cw { position: fixed; right: 16px; bottom: calc(72px + env(safe-area-inset-bottom, 0px)); z-index: var(--z-toast, 9000); display: flex; flex-direction: column; align-items: flex-end; gap: 10px; }
