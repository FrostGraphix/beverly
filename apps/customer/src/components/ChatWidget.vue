...
const view        = ref<'chat' | 'escalate'>('chat'); // 'chat' | 'escalate'
const sessionId   = ref('');
...
async function fetchMessages(initial = false) {
    ...
    try {
        ...
        if (initial) {
            ...
            // Remove optimistic duplicates then push real messages
            messages.value = messages.value.filter((m) => !m._pending);
            messages.value.push(...incoming);
            const newStaff = incoming.filter((m) => m.sender_actor_type !== 'customer');
            if (!open.value) unread.value += newStaff.length;
            if (newStaff.length) {
                // Clear typing when real message arrives
...
async function submitEscalate() {
    ...
    try {
        ...
        const res = await api.post<{ reference: string }>(
            `/api/v1/customer/support/chat/${sessionId.value}/escalate`,
            payload,
        );
        escDone.value = res;
        messages.value.push({
        ...
        <template v-if="view === 'chat'">
          <div ref="scroller" class="cw-body">
            ...
            <div
              ...
              :class="['cw-msg',
                m.sender_actor_type === 'customer' ? 'cw-msg--mine' : m.kind === 'system' ? 'cw-msg--sys' : 'cw-msg--them',
        ...
        <template v-else>
          <div class="cw-esc-body">
            ...
            <input
              v-model="escSubject"
              class="cw-esc-input"
              placeholder="e.g. Token not received"
              maxlength="120"
...
/* Mine: green bubble, top-right corner stays rounded, bottom-right cuts */
.cw-msg--mine .cw-msg-body {
  background: linear-gradient(180deg, var(--brand-300), var(--brand-600));
  color: #04140b;
  border-bottom-right-radius: 4px;
}
.cw-msg--mine .cw-msg-body.cw-msg-body--last { border-bottom-right-radius: 4px; }
/* Theirs: surface bubble */
