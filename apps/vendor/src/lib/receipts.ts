...
export type ReceiptModel = {
  ...
  issuedAt?: string;
  subject?: string;
  source?: { kind: 'wallet-ledger'; id: string };
  subtitle?: string;
  fields: ReceiptField[];
};
...
function resolveVendedBy(row: any): string {
  if (row.vended_by && !row.vended_by.startsWith('Authorized Vendor Operator') && !row.vended_by.startsWith('Authorized System Actor')) {
  ...
  if (name && business && String(name).trim() !== String(business).trim()) {
    return `${name} — ${business}${station ? ` (${station})` : ''}`;
  }
  if (name || business) {
    return `${name || business}${station ? ` (${station})` : ''}`;
  }
  ...
  try {
    if (typeof localStorage !== 'undefined' || typeof sessionStorage !== 'undefined') {
      const rawUser = (typeof sessionStorage !== 'undefined' && sessionStorage.getItem('beverly.vendor.user'))
        || (typeof localStorage !== 'undefined' && localStorage.getItem('beverly.vendor.user'))
        || (typeof localStorage !== 'undefined' && localStorage.getItem('beverly_vendor_session'));
      if (rawUser) {
        const u = JSON.parse(rawUser);
        const userObj = u.user || u;
        const vName = userObj.full_name || userObj.name || userObj.contact_person || '';
        const vBiz = userObj.organization_name || userObj.trading_name || userObj.legal_name || userObj.business_name || '';
        const vStation = userObj.site || userObj.station_id || station || '';
        if (vName && vBiz) {
          return `${vName} — ${vBiz}${vStation ? ` (${vStation})` : ''}`;
        }
        if (vName || vBiz) {
          return `${vName || vBiz}${vStation ? ` (${vStation})` : ''}`;
        }
  ...
  return 'Authorized Vendor Operator';
}

export function purchaseReceipt(row: any): ReceiptModel {
  const vendedBy = resolveVendedBy(row);
  ...
  return {
    ...
    fields: [
      ...
      field('Purchase Way', row.purchase_way || (row.mode === 'remote_send' ? 'Wireless Remote Send' : 'Vendor Portal Vending')),
...
export function ledgerReceipt(entry: any): ReceiptModel {
  ...
  return {
    ...
    amount: `${entry.direction === 'credit' ? '+' : '-'}${money(entry.amount_minor)}`,
    status: direction,
    issuedAt: date(entry.created_at),
    subject: 'Ledger Amount',
    source: { kind: 'wallet-ledger', id: clean(entry.id) },
    fields: [
      field('Entry Type', clean(entry.entry_type).replace(/_/g, ' ')),
