...
### 23.48 Offline credit safety and live-status verification (2026-09-25)

- A fresh, authenticated, read-only customer request reconfirmed `ACOB_AJEGUNLE_034B`, meter `SMRSD-04-000844F5`, automatic operating mode, NGN 0.00 credit, NGN 0.00 plan balance, NGN 0.00 technical debt, and the NGN 400/kWh Residential FLAT tariff. No provider or database write occurred.
- Koios v1's customer response does not expose last-reading time, heartbeat, current relay state, voltage, or current. The supplied portal login did not establish an authenticated session, so current physical connectivity and relay state remain unverified. Customer metadata must not be represented as live-meter proof.
- Red adapter tests now cover stale telemetry, a zero-credit energized meter, a negative provider balance, and fresh positive credit. The adapter fails closed after thirty minutes without telemetry, blocks non-automatic modes, flags energized zero-credit meters, and never displays projected credit below zero.
- This guard does not claim Beverly can disconnect a physically offline meter. Guaranteed offline zero-credit enforcement requires the SparkMeter meter firmware and relay to enforce prepaid cutoff locally. On reconnection, Beverly must read provider truth and reconcile confirmed cumulative energy without estimated charges or automatic payment retries.
- Focused adapter tests pass with 19 tests. The wallet production build passes. Calinmeter code and behavior remain unchanged.

### 23.49 Runtime audit correction (2026-09-26)

- Correction to checkpoint 23.48: the credit helper has no production caller. Its passing tests never proved runtime write blocking, physical cutoff, or debt prevention. The portal login stayed on its login page; credential rejection itself was not established by an explicit error.
- The assessment now always returns `allowFinancialWrite=false`. It preserves negative provider credit, reports malformed balances as unknown, rejects malformed or future telemetry, and treats positive credit as unknown physical relay state. Thirty minutes is an advisory display policy, not a certified provider safety threshold.
- Installation resolution now rejects missing tenant authority and out-of-scope installation requests before storage access, applies tenant/installation filters to database queries, and rejects explicit identity substitution. Database-transport harness tests are not proof of deployed RLS.
- The operator inventory reader now rejects malformed meter collections, provider error envelopes, malformed/repeated cursors and page-budget exhaustion. Read retries are bounded, use exponential delays, respect `Retry-After`, suppress transport payload details, and reject redirects. Response-body consumption retains its timeout signal.
- The deeper audit confirms remaining implementation gaps: trusted runtime tenant membership, installation-scoped legacy storage, executed provider gateway, canonical telemetry schemas, atomic ingestion/checkpoints, scheduler/replay, downstream scoped reads, credential rotation, write dispatch/reconciliation and live deployment smoke. These are not all external blockers.
- `docs/OEM_PIPELINE_EXECUTION_STATUS.md` records the ordered plan and current verification. SparkMeter remains draft; no financial dispatch or production activation was performed.
- Verification: the complete wallet suite passed on Node 22.23.2 (78 files, 538 tests), along with wallet typecheck/build. Root regression and all six application/backend builds passed on Node 24.13.1. Import regressions passed after the final malformed-response redaction change. Wallet lint failed because ESLint is absent; protected-preview smoke lacks its bypass credential. No new database migration was executed.
- A subsequent complete read-only ACOB inventory scan validated 3,112 customers and 3,073 meters; 39 meterless customers remain excluded. This is one additional metered customer compared with checkpoint 23.46. No provider or database write occurred, and current production mappings were not reconciled against this changed inventory.
