...
function main() {
  ...
  assert(adminRoutes.includes("reachability === 'email'"), "Email-only sends must filter to reachable addresses.");
  assert(adminRoutes.includes("channels: z.array(z.enum(['in_app', 'email']))"), "Announcement channels must be validated.");
  assert(adminRoutes.includes("const wantsNotifications = channels.includes('in_app')"), "Notification delivery must be independently selectable.");
  ...
  assert(adminPage.includes("Choose delivery medium"), "Delivery-medium step missing.");
  assert(adminPage.includes("Notification") && adminPage.includes("Email") && adminPage.includes("Both"), "All delivery-medium choices are required.");
  ...
  assert(adminPage.includes("Sent announcements cannot be recalled."), "Send warning missing.");
  assert(adminPage.includes("channels: channelPayload.value"), "Selected delivery channels must reach the backend.");
  ...
  assert(adminPage.includes("audience: 'system', delivery: deliveryMode.value, limit: '1'"), "Announcement stats must follow the selected delivery medium.");
  assert(adminPage.includes('role="tablist"'), "Compose and history must be separate accessible tabs.");
  assert(adminPage.includes("an-history-grid"), "Message history must render as a compact card grid.");
  assert(adminPage.includes("openHistoryDetail(item, $event)"), "History cards must open their full content.");
  assert(adminPage.includes('class="an-detail-dialog"'), "History details must use a dedicated dialog.");
  assert(adminPage.includes("historyDetailClose.value?.focus()"), "History details must move focus into the dialog.");
  assert(adminPage.includes("historyTrigger?.focus()"), "Closing history details must restore focus.");
  ...
  assert(vendorRoutes.includes(".not('notification_id', 'is', null)"), "Email-only deliveries must stay out of the vendor inbox.");
  ...
  assert(customerRoutes.includes(".not('notification_id', 'is', null)"), "Email-only deliveries must stay out of the customer inbox.");
