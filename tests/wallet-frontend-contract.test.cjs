...
function main() {
  ...
  const adminVending = read("apps/admin/src/views/Vending.vue");
  ...
  const fundingCallbacks = read("backend/wallet/src/config/funding-callbacks.ts");
  ...
  assert(fundWallet.includes("/api/v1/customer/payments/"));
  assert(vendorFund.includes("/api/v1/vendor/payments/"));
  assert(fundWallet.includes("payment.fulfillmentStatus"));
  assert(vendorFund.includes("payment.fulfillmentStatus"));
  assert(fundWallet.includes("do not pay again"));
  assert(vendorFund.includes("do not pay again"));
  assert(!buyToken.includes("direct_pay"));
  assert(!buyToken.includes("Pay with Card"));
  assert(buyToken.includes('to="/wallet/fund"'));
  assert(buyToken.includes("Fund wallet"));
  assert(customerRoute.includes("mode: z.literal('wallet')"));
  assert(!customerPurchaseService.includes("customer.purchase.direct_pay.init"));
  assert(!customerPurchaseService.includes("purpose: 'token_purchase'"));
  assert(buyToken.includes("Token generated!"));
  assert(buyToken.includes("remoteSendGeneratedToken"));
  assert(buyToken.includes("downloadResultReceipt"));
  assert(customerTransactions.includes("Payment needs review"));
  assert(customerTransactions.includes("Token generated; reconciling"));
  assert(adminVending.includes("Payment needs review"));
  assert(adminVending.includes('class="bw-btn sm vending-filter-button"'));
  assert(adminVending.includes('aria-controls="vending-filters"'));
  assert(adminVending.includes('setTimeout(() => void load(), 300)'));
  assert(adminVending.includes('Clear filters'));
  assert(adminDashboard.includes("Payment needs review"));
  assert(adminDashboard.includes('aria-label="Loading dashboard metrics"'));
  assert(adminDashboard.includes('aria-busy="true"'));
  assert(adminDashboard.includes("dashboard-kpi-skeleton-chart"));
  assert(customerRoute.includes("CUSTOMER_FUNDING_CALLBACK_URL"));
  assert(vendorRoute.includes("VENDOR_FUNDING_CALLBACK_URL"));
  assert(fundingCallbacks.includes("wallet/fund"));
  ...
  assert(walletEnvExample.includes("https://customer-acob-beverly.vercel.app/wallet/fund?payment=return"));
  assert(walletEnvExample.includes("https://vendor-acob-beverly.vercel.app/wallet/fund?payment=return"));
  assert(vercelConfig.includes("https://customer-acob-beverly.vercel.app/wallet/fund?payment=return"));
  assert(vercelConfig.includes("https://vendor-acob-beverly.vercel.app/wallet/fund?payment=return"));
  ...
  assert(!adminShell.includes('title="Toggle theme"'));
  assert(!vendorShell.includes('title="Toggle theme"'));
  assert(adminShell.includes('to="/support"'));
  assert(vendorShell.includes('to="/help"'));
