...
function main() {
  ...
  assert(customerApi.includes("const hasBody = body !== undefined"));
  assert(customerApi.includes("if (hasBody) headers['Content-Type']"));
  assert(customerApi.includes("body: hasBody ? JSON.stringify(body) : undefined"));
  assert(vendorApi.includes("redirectToPayment"));
  assert(vendorApi.includes("checkout.paystack.com"));
  assert(vendorApi.includes("const hasBody = body !== undefined"));
  assert(vendorApi.includes("if (hasBody) headers['Content-Type']"));
  assert(vendorApi.includes("body: hasBody ? JSON.stringify(body) : undefined"));
  ...
  assert(vendorDashboard.includes("entry.entry_type.startsWith('reversal_')"));
  assert(vendorDashboard.includes("activityFilter"));
  assert(vendorWallet.includes("viewMode === 'cards'"));
  assert(vendorWallet.includes("viewMode === 'table'"));
  assert(vendorWallet.includes(':aria-pressed="viewMode === \'table\'"'));
  assert(vendorWallet.includes("viewLedgerReceipt(e)"));
  assert(vendorWallet.includes("printLedgerReceipt(e)"));
  assert(vendorWallet.includes("downloadLedgerReceipt(e)"));
  assert(vendorReceipts.includes("export function ledgerReceipt"));
  assert(vendorReceipts.includes("title: 'Wallet Ledger Receipt'"));
  assert(vendorShell.includes('class="bw-install-toast"'));
  assert(vendorShell.includes('class="bw-install-logo"'));
  assert(buyToken.includes("/api/v1/customer/payments/"));
  assert(fundWallet.includes("/api/v1/customer/payments/"));
  assert(vendorFund.includes("/api/v1/vendor/payments/"));
  assert(vendorFund.includes('for="vendor-fund-amount"'));
  assert(vendorFund.includes(':aria-pressed="mode ==='));
  assert(vendorFund.includes('role="alert"'));
  assert(vendorFund.includes('PROOF_MIME_TYPES'));
  assert(vendorFund.includes('fundingLoadError'));
  assert(buyToken.includes("payment.fulfillmentStatus"));
  assert(fundWallet.includes("payment.fulfillmentStatus"));
  assert(vendorFund.includes("payment.fulfillmentStatus"));
  assert(buyToken.includes("Do not pay again"));
  assert(fundWallet.includes("do not pay again"));
  assert(vendorFund.includes("do not pay again"));
  assert(customerRoute.includes("/buy-token?payment=return"));
  assert(customerRoute.includes("/wallet/fund?payment=return"));
