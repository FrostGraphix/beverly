...
const route = read('backend/wallet/src/routes/admin.ts') + read('backend/wallet/src/routes/admin-reports.ts');
...
assert.match(page, /Customer-owned activity/);
assert.match(page, /Delivered revenue/);
...
assert.match(route, /family === 'audit' && !ownerScope \? readAllRows\('Audit logs'/);
...
assert.match(route, /throw new ReportQueryError\(`\$\{source\} report query failed:/);
