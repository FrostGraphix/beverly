...
assert.ok(shell.includes("const isSuperAdmin = computed(() => auth.user?.role === 'super-admin');"), "CRM return must be Super Admin only");
assert.match(shell, /v-if="isSuperAdmin" class="bw-sidebar-foot"/, "sidebar CRM return must be gated");
