...
import {
    isDisposableEmail,
    isCorporateStaffEmail,
    validateEmailFormatAndDomain,
    EmailValidationError,
} from '../backend/wallet/dist/services/email-validation.js';
...
async function runTests() {
    ...
    await assert.rejects(
        ...
        (err) => err instanceof EmailValidationError && err.code === 'invalid_email',
        'Invalid syntax should throw invalid_email'
    );

    await assert.rejects(
        async () => validateEmailFormatAndDomain('spammer@tempmail.com'),
        (err) => err instanceof EmailValidationError && err.code === 'disposable_email_not_allowed',
