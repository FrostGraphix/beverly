...
as $$
  with current_rows as materialized (
    select mca.station_id, mca.meter_id, mca.customer_id, mca.customer_name,
           mca.period_start, mca.kwh_total, mca.reading_count,
           coalesce(binding.tariff_id, '') as tariff_id
    from public.meter_consumption_aggregates mca
    left join lateral (
      select ab.tariff_id
      from public.account_bindings ab
      where ab.meter_id = mca.meter_id
        and (ab.customer_id = mca.customer_id or coalesce(mca.customer_id, '') = '')
      order by (ab.customer_id = mca.customer_id) desc,
               (upper(coalesce(ab.station_id, '')) = upper(mca.station_id)) desc,
               ab.updated_at desc nulls last
      limit 1
    ) binding on true
    where mca.period_type = p_period_type
      and mca.period_start between p_from and p_to
      and (p_station_id is null or upper(mca.station_id) = upper(p_station_id))
      and upper(mca.station_id) = any(array['TUNGA','UMAISHA','OGUFA','KYAKALE','MUSHA'])
  ),
  ...
  station_rows as (
    select c.station_id,
           ...
           count(distinct coalesce(nullif(c.customer_id, ''), c.meter_id)) as customer_count,
  ...
  tariff_rows as (
    select tariff_id, sum(kwh_total) as total_kwh
    from current_rows
    group by tariff_id
  ),
  top_rows as (
  ...
  select jsonb_build_object(
    ...
    'tariffBreakdown', coalesce((
      select jsonb_agg(to_jsonb(tr) order by tr.total_kwh desc)
      from tariff_rows tr
    ), '[]'::jsonb),
    'topMeters', coalesce((
