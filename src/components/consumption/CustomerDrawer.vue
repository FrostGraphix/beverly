    ...
    <div v-if="customer" class="drawer-overlay" @click.self="$emit('close')">
      <div class="drawer-panel" role="dialog" aria-label="Customer Detail">
        ...
        <div class="drawer-tabs">
          <button v-for="t in tabs" :key="t.key" :class="['drawer-tab', activeTab === t.key ? 'active' : '']" @click="activeTab = t.key">{{ t.label }}</button>
        </div>
        ...
        <div class="drawer-footer">
          <button class="footer-btn footer-btn--outline" @click="exportPdf">Export PDF</button>
          <button class="footer-btn footer-btn--danger" @click="$emit('relay-control', customer)">
    ...
    async renderDeltaChart() {
      ...
      this._chart.setOption({
        ...
        series: [{
          ...
          areaStyle: { color: { type: "linear", x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: "#36a3f750" }, { offset: 1, color: "#36a3f705" }] } },
...
.drawer-panel {
  width: 480px; max-width: 96vw; background: var(--bg-card);
...
.drawer-head { display: flex; align-items: center; justify-content: space-between; padding: 18px 18px 14px; border-bottom: 1px solid var(--border-color); }
.drawer-head-info { display: flex; align-items: center; gap: 12px; }
.drawer-avatar { width: 44px; height: 44px; border-radius: 50%; background: var(--primary); color: #fff; font-weight: 700; font-size: 15px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.drawer-close:hover { border-color: #f4516c; color: #f4516c; }
...
.tariff-pill { background: rgba(255,184,34,.12); color: #ffb822; }
.risk-banner { display: flex; align-items: center; gap: 10px; padding: 10px 18px; font-size: 12px; }
.risk-banner.risk-low { background: rgba(52,191,163,.1); color: #34bfa3; }
.risk-banner.risk-medium { background: rgba(255,184,34,.1); color: #ffb822; }
.risk-banner.risk-high { background: rgba(244,81,108,.1); color: #f4516c; }
.risk-banner.risk-critical { background: rgba(170,0,255,.1); color: #aa00ff; }
...
.drawer-kpi-row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 0; border-bottom: 1px solid var(--border-color); }
...
.drawer-kpi.danger .drawer-kpi-v { color: #f4516c; }
.drawer-kpi.success .drawer-kpi-v { color: #34bfa3; }
.drawer-tabs { display: flex; border-bottom: 1px solid var(--border-color); }
.drawer-tab { flex: 1; padding: 10px; font-size: 12px; font-family: var(--font-family); border: none; background: transparent; cursor: pointer; color: var(--text-muted); border-bottom: 2px solid transparent; transition: all 0.15s; }
...
.recharge-table { width: 100%; border-collapse: collapse; font-size: 12px; }
.recharge-table th { padding: 8px 10px; text-align: left; font-size: 11px; color: var(--text-muted); border-bottom: 1px solid var(--border-color); }
.recharge-table td { padding: 8px 10px; border-bottom: 1px solid var(--border-color); color: var(--text-main); }
.recharge-table tr:hover { background: var(--primary-light); }
...
.token-stat--used { color: #34bfa3; font-weight: 700; }
.token-stat--unused { color: #ffb822; font-weight: 700; }
.vend-badge { padding: 2px 8px; border-radius: 10px; font-size: 10px; font-weight: 700; letter-spacing: 0.03em; }
.vend-badge--used { background: rgba(52,191,163,.12); color: #34bfa3; }
.vend-badge--unused { background: rgba(255,184,34,.12); color: #e6a817; }

.drawer-footer { display: flex; gap: 10px; padding: 14px 18px; border-top: 1px solid var(--border-color); }
.footer-btn { flex: 1; padding: 9px 0; border-radius: var(--radius-md); font-size: 12px; font-family: var(--font-family); cursor: pointer; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.15s; }
...
.footer-btn--danger { background: #f4516c; border: none; color: #fff; }
.footer-btn--danger:hover { background: #e0384d; }
